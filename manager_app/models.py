...
class Reservation(models.Model):
    ...
    def table_is_free(self):
        ...
        if self.table:
            table_reservations = Reservation.objects.filter(
                date=self.date, table=self.table
            ).exclude(id=self.id)
            for res in table_reservations:
                if res.hour <= self.hour < res.end_hour:
                    raise ValidationError(
                        f"{self.table} jest już zajęty, jest zarezerwowany na {res.hour}"
                    )
...
class ExtraInfo(models.Model):
    ...
    vegetarian = models.PositiveIntegerField(
        default=0, verbose_name="Dieta wegetariańska"
    )
    ...
    peanut_allergy = models.PositiveIntegerField(
        default=0, verbose_name="Alergia na orzechy"
    )
    dairy = models.PositiveIntegerField(default=0, verbose_name="Dieta beznabiałowa")
    child_seat = models.PositiveIntegerField(
        default=0, verbose_name="Krzesełko dla dziecka"
    )
    ...
    def get_fields(self):
        return [
            ...
            for field in ExtraInfo._meta.fields
        ]
